---
layout: post
title: "Introducing Trinity: Two-Party Computation for the ZK Era"
date: 2025-04-15
---

**Trinity** is a two-party computation (2PC) protocol that brings together garbled circuits and KZG commitments, enabling reusable and verifiable inputs for secure computation. It's designed to bridge the gap between MPC and ZK tooling.
...
- **Garbled Circuits (GC)**: One party (the *garbler*) encrypts the entire circuit. Each wire in the circuit gets two cryptographic labels (representing 0 or 1), and the circuit gates are encrypted so that only the correct output label can be unlocked, step by step. The other party (the *evaluator*) evaluates this encrypted circuit without ever seeing the real bits.
- **Oblivious Transfer (OT)**: The evaluator (Alice) needs to receive *just* the labels corresponding to her input bits — but without revealing those bits to the garbler.
...
Now Alice can receive the garbled data from the Bob and evaluate the circuit. Note that the process is asymmetric, and only Alice evaluates the circuit on her side and can send back the result to Bob.
...
const resultBytes = currentEvaluator.evaluate(bundle, currentProtocol?.circuit_parsed);
// Get the result
